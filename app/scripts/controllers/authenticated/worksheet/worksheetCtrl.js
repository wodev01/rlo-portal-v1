...
app.controller('worksheetCtrl',
    function ($scope, $http, $timeout, $mdSidenav, toastr, $log, $rootScope, $cookies, $state, $mdDialog, $filter, worksheetServices) {
        ...
        $scope.openDatePicker = function (ev, newWorksheet,dateField) {
            ...
            var DatePickerDialogController = ['$scope', '$mdDialog',function ($scope, $mdDialog) {
                if(dateField === 'period_start'){
                    if(newWorksheet.period_start){$scope.dateValue = newWorksheet.period_start;}
                }else if(dateField === 'period_end'){
                    if(newWorksheet.period_end){$scope.dateValue = newWorksheet.period_end;}
                }
        ...
        $scope.worksheetAction = '<div layout="row"><md-button class="md-icon-button md-warn md-hue-2" ng-click="grid.appScope.fnWorksheetDelete(row,$event)"><md-icon md-font-set="material-icons">delete</md-icon><md-tooltip md-direction="top">Remove</md-tooltip></md-button>' +
            '<md-button class="md-icon-button md-accent" ng-click="grid.appScope.openWorksheet(row,$event)"><md-icon md-font-set="material-icons">visibility</md-icon><md-tooltip md-direction="top">Open</md-tooltip></md-button></md-button></div>';
        $scope.worksheetGridOptions = {
            ...
            columnDefs: [
                {field: 'period_start', displayName: 'Month And Year',minWidth:100,cellFilter: 'date:\'MM-yyyy\'', enableHiding: false},
                ...
                {name: 'action', displayName:'', cellTemplate: $scope.worksheetAction, width:100, enableSorting:false, enableColumnMenu: false}
            ]
        };
