	...
	function ($scope, $rootScope, $mdDialog, $filter, $timeout, $location,
		...
		$scope.fnGetLocationDetails = function () {
			...
			locationService.fetchLocation().then(function (response) {
				...
				$scope.fnLocationStatus(response);
			}, function (error) {
		...
		/*-------------Location Status---------------*/
		$scope.fnLocationStatus = function (data) {
			if (data.length == 0) {
				var confirm = $mdDialog.confirm()
					.title('Create Location')
					.content('No locations exist. Please create location.')
					.ariaLabel('Ok')
					.ok('Create')
					.cancel('Cancel');
				...
				$mdDialog.show(confirm).then(function () {
					...
					$rootScope.isOpenNewLocation = true;
				}, function () {
				});
			}
		};
		...
		$scope.fnCustomDialog = function () {
			var DialogController = ['$scope', '$mdDialog', function ($scope, $mdDialog) {
				...
				$scope.fnGoTo = function () {
					$scope.fnHide();
					$location.url('/location/setup');
				};
			}];

			$mdDialog.show({
				controller: DialogController,
				template: '<md-dialog id="download-app" aria-label="downloadApp">' +
				'   <md-toolbar class="md-primary">' +
				'       <div class="md-toolbar-tools">Location Setup</div>' +
				'   </md-toolbar>' +
						...
						onclick: function (d) {
							if (d.name) {
								...
								if ($rootScope.isMobile) {
									toastr.info('Please put your finger on the specific target.');
								}
							}
